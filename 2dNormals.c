...
int findNormal(double p1[2], double p2[2], double padd[2]) {
  ...
  xslope = p2[0] - p1[0];
  yslope = p2[1] - p1[1];
  uvec = sqrt(pow(xslope, 2) + pow(yslope, 2));
  xslope = xslope / uvec;
  yslope = yslope / uvec;
  padd[0] = p1[0] + (yslope) * 25;
  padd[1] = p1[1] - (xslope) * 25;
}
